...
def project_create(config=None, name=None, description='', goals='', tags=[], access='private', group='computational'):
    ...
    request['group'] = group
    conn.request("POST","/api/v0.1/private/{0}/no-app/project/create".format(config['api']['key']), json.dumps(request), headers)
    response = conn.getresponse()
    data = response.read()
    conn.close()
    try:
...
def record_create(config=None, project=None, request={}):
    ...
    headers = {"Accept": "application/json"}
    conn.request("POST","/api/v0.1/private/{0}/no-app/project/record/create/{1}".format(config['api']['key'], project), json.dumps(request), headers)
    response = conn.getresponse()
    data = response.read()
    conn.close()
    try:
...
def record_update(config=None, record=None, request={}):
    ...
    headers = {"Accept": "application/json"}
    conn.request("POST","/api/v0.1/private/{0}/no-app/record/update/{1}".format(config['api']['key'], record), json.dumps(request), headers)
    response = conn.getresponse()
    data = response.read()
    conn.close()
    try:
...
def upload_file(config=None, path=None, obj=None, group=None):
    if api_status(config=config) :
        if path and group and obj:
            url = "http://{0}:{1}/api/v0.1/private/{2}/no-app/file/upload/{3}/{4}".format(config['api']['host'], int(config['api']['port']), config['api']['key'],group,obj)
        else:
...
def project_env_next(config=None, project=None, path=None):
    ...
    conn.request("POST","/api/v0.1/private/{0}/no-app/project/env/next/{1}".format(config['api']['key'], project), json.dumps(request), headers)
    response = conn.getresponse()
    data = response.read()
    conn.close()
    try:
        data_json = json.loads(data)
        if data_json['code'] == 201:
            upload_response = upload_file(config=config, path=path, obj=data_json['content']['bundle'], group='bundle')
